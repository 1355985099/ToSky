...
<body oncontextmenu="return false">
		...
		<form id="commentForm" action="${app}/login/reg" method="post">
			...
			<div>
				<input id="password" type="password" name="password"
					class="password" placeholder="密码" value="${user.password}" />
			</div>
			<div>
				<input id="password2" type="password" name="password2"
					class="password2" placeholder="确认密码" value="${user.password2}" />
			</div>
			<div>
				<input id="tel" type="text" name="tel" class="username"
					placeholder="电话" value="${user.tel}" />
			</div>
			<div>
				<input id="email" type="text" name="email" class="username"
					placeholder="邮箱" value="${user.email}" /> <input id="emailr"
					type="hidden" value="" />
			</div>
	...
	<script>
		...
		jQuery(function($) {
			...
			/*数据库查重*/
			jQuery.validator.addMethod("isRepeat",
					function(value, element) {
				
						var re;
						$.ajax({
							url : "${app}/login/isRepeat",
							async : false,//改为同步方式 
							type : "POST",
							data : {
								vf : $(element).attr("name")+","+value
							},
